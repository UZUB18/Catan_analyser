...
@click.option(
    ...
    default=False,
    is_flag=True,
    help="Run games in parallel using all CPU cores for faster simulation.",
)
@click.option(
    "--workers",
    default=None,
...
def _play_single_game(args):
    """Play a single game - used for parallel execution."""
    from catanatron.cli.cli_players import CLI_PLAYERS

    game_config, player_codes = args

    # Rebuild players from codes for this process
    colors = [Color.RED, Color.BLUE, Color.ORANGE, Color.WHITE]
    players = []
    for i, code_parts in enumerate(player_codes):
        parts = code_parts.split(":")
        code = parts[0]
...
def play_batch_parallel(
    ...
    """Run games in parallel using multiple CPU cores."""
    output_options = output_options or OutputOptions()
    game_config = game_config or GameConfigOptions()
    ...
    # Convert players to serializable codes (e.g., "R", "W", "G:5")
    player_codes = []
    for player in players:
        # Map class name back to code
        from catanatron.cli.cli_players import CLI_PLAYERS

        for cli_player in CLI_PLAYERS:
            if cli_player.import_fn == player.__class__:
                player_codes.append(cli_player.code)
                break
        else:
            # Fallback: use class name
            player_codes.append(player.__class__.__name__)

    console.print(
        f"[green]Running {num_games} games in parallel using {workers} workers...[/green]"
    )

    # Prepare args tuple for each game
    game_args = [(game_config, player_codes) for _ in range(num_games)]
    ...
    with Pool(processes=workers) as pool:
        with Progress(
            ...
            task = progress.add_task(f"Playing {num_games} games...", total=num_games)

            for result in pool.imap_unordered(_play_single_game, game_args):
    ...
    turns = []
    ticks = []
    vps_by_color = defaultdict(list)

    for result in results:
        if result["winner"]:
            wins[Color(result["winner"])] += 1
        turns.append(result["turns"])
        ticks.append(result["ticks"])
        for color_val, vp in result["vps"].items():
            vps_by_color[Color(color_val)].append(vp)
    ...
    for i, player in enumerate(players):
        avg_vps = (
            sum(vps_by_color[player.color]) / len(vps_by_color[player.color])
            if vps_by_color[player.color]
            else 0
        )
        table.add_row(
            rich_player_name(player),
            str(wins[player.color]),
            f"{avg_vps:.2f}",
        )
    console.print(table)

    # ===== GAME SUMMARY
    avg_ticks = f"{sum(ticks) / len(ticks):.2f}" if ticks else "0"
    avg_turns = f"{sum(turns) / len(turns):.2f}" if turns else "0"
    table = Table(box=box.MINIMAL, title="Game Summary")
    table.add_column("AVG TICKS", justify="right")
    table.add_column("AVG TURNS", justify="right")
    table.add_column("GAMES", justify="right")
    table.add_row(avg_ticks, avg_turns, str(len(results)))
    console.print(table)

    return dict(wins), dict(vps_by_color), results
